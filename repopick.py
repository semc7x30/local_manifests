...
if args.topic:
    for argument in args.topic:
        ...
        if 'LAOS' in gerrit:
            ...
            url = 'https://review.msm7x30.org/changes/?q=topic:%s' % tag
...
if args.query:
    for argument in args.query:
        ...
        if 'LAOS' in gerrit:
            ...
            url = 'https://review.msm7x30.org/changes/?q=%s' % pquery
        if args.verbose:
...
for argument in args.change_number:
    ...
    if 'LAOS' in gerrit:
        ...
        url = 'https://review.msm7x30.org/changes/?q={change}&o={query_revision}&o=CURRENT_COMMIT&pp=0'.format(change=change, query_revision=query_revision)
    if args.verbose:
